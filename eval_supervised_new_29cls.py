...
def main(local_rank, args):
    ...
    MAPPING16_TO_29 = torch.tensor([0, 9, 14, 16, 17, 18, 21, 2, 12, 22, 23, 24, 25, 26, 27, 28, 29, 30]).cuda()
    ...
    SemKITTI_label_name = get_nuScenes_label_name("config/label_mapping/nuscenes-29cls.yaml")
    unique_label = list(range(1,30)) #np.asarray(cfg.unique_label)
    ...
    unique_label_clip = [i for i in range(30)]
    SemKITTI_label_name_clip = get_nuScenes_label_name("./config/label_mapping/nuscenes-29cls.yaml")
    ...
    CalMeanIou_vox_all = MeanIoU(unique_label_clip + [EMPTY_SEMANTIC_LABEL], IGNORE_LABEL, unique_label_str_clip + ['empty'],
    ...
    if cfg.resume_from and osp.isfile(cfg.resume_from):
        ...
        print(my_model.load_state_dict(revise_fnc(ckpt, ddp=distributed and not args.no_dist),
                                       strict=False))
    elif cfg.load_from:
    ...
    with torch.no_grad():
        for i_iter_val, loaded_data in enumerate(val_dataset_loader):
            ...
            time_s = time.time()
            imgs, img_metas, val_vox_label_semantic, val_grid, val_pt_labs, val_vox_label_cls, val_vox_label_cls_val,*_ = loaded_data
            ...
            predict_labels_pts_semantic = MAPPING16_TO_29[predict_labels_pts_semantic]
            ...
            predict_labels_vox_semantic = MAPPING16_TO_29[predict_labels_vox_semantic]
