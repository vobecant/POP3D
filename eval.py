...
from eval_maskclip import EMPTY_SEMANTIC_LABEL
from qualitative_results import res2txt, get_clip_utils
...
from tmp_visualizer import PICKED
...
def main(local_rank, args):
    ...
    # TODO: 1) LIMIT VAL LOADER
    infos = val_dataset_loader.dataset.imagepoint_dataset.nusc_infos
    kept_infos = []
    for info in infos:
        token = info['token']
        if token in PICKED:
            kept_infos.append(info)
    val_dataset_loader.dataset.imagepoint_dataset.nusc_infos = kept_infos

    # TODO: 2) 3x SCALE
    # TODO: 3) Inference the original TPVFormer
    ...
    class_mapping_clip, text_features, colors = get_clip_utils(args, val_dataset_loader, no_feats=True)
    ...
    with torch.no_grad():
        for i_iter_val, loaded_data in enumerate(val_dataset_loader):
            ...
            print(f'{i_iter_val}it, input {val_vox_label.unique(return_counts=True)}')
            ...
            predict_labels_vox = predict_labels_vox[0].argmax(0)
            xyz_pred = torch.stack(torch.where(predict_labels_vox.cpu() != EMPTY_SEMANTIC_LABEL))
            labels_pred = predict_labels_vox[xyz_pred[0],xyz_pred[1],xyz_pred[2]]

            xyz_pred = vox2real(xyz_pred)

            token = img_metas[0]['token']
            save_dir_cur = os.path.join(args.save_dir, token)
            ...
            print(f'{i_iter_val}it, prediction {predict_labels_pts.unique(return_counts=True)}, '
                  f'target {val_pt_labs.unique(return_counts=True)}\n')
            ...
            locations_for_vox = val_grid_int if not args.unique_voxels else torch.stack(
                torch.where(vox_label != cfg.dataset_params['fill_label'])).T

            if args.agnostic:
                predict_labels_vox_agn = copy.deepcopy(predict_labels_vox)
                val_pt_labs_agn = copy.deepcopy(val_pt_labs)
                predict_labels_pts_agn = copy.deepcopy(predict_labels_pts)
                vox_label_agn = copy.deepcopy(vox_label)
            else:
            ...
            for count in range(len(val_grid_int)):
                ...
                if args.unique_voxels:
                    ...
                    CalMeanIou_vox_agn_unq._after_step(
                        predict_labels_vox_agn[count, locations_for_vox[cur_indices, 1],
                                               locations_for_vox[cur_indices, 2],
                                               locations_for_vox[cur_indices, 3]].flatten().cuda(),
                        vox_label_agn[count, locations_for_vox[cur_indices, 1], locations_for_vox[cur_indices, 2],
                                      locations_for_vox[cur_indices, 3]].flatten().cuda()
                    )
                    CalMeanIou_vox_unq._after_step(
                        predict_labels_vox[count, locations_for_vox[cur_indices, 1], locations_for_vox[cur_indices, 2],
                                           locations_for_vox[cur_indices, 3]].flatten().cuda(),
                        vox_label[count, locations_for_vox[cur_indices, 1], locations_for_vox[cur_indices, 2],
                                  locations_for_vox[cur_indices, 3]].flatten().cuda())
