    ...
    with torch.no_grad():
        for i_iter_val, loaded_data in enumerate(val_dataset_loader):
            ...
            for _i in range(occupied_voxels_loc.shape[1]):
                labels = val_pt_fts[:, inv_idx == _i].squeeze().int()
                if len(labels.shape) == 0:
                    most_frequent = labels
                else:
                    most_frequent = torch.bincount(labels).argmax()
                predicted_features_occupied_vox.append(most_frequent)
            predicted_features_occupied_vox = torch.tensor(predicted_features_occupied_vox).unsqueeze(0)
            ...
            outputs_vox_clip = predicted_features_occupied_vox
            ...
            if clip_features:
                ...
                with torch.no_grad():
                    ...
                    # projected points
                    outputs_pts_clip = val_pt_fts[0].long()
                    outputs_pts_clip_all = torch.ones_like(val_pt_labs) * 0 #EMPTY_SEMANTIC_LABEL
