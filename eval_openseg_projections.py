...
def assign_labels_clip(predicted_features, text_features, class_offset, maskclip=False, class_mapping_clip=None,
    ...
    if class_mapping_clip is not None:
        class_mapping_clip = torch.tensor(class_mapping_clip).cuda()
    if maskclip:
    ...
    elif normalized_cosine:
        predicted_features_norm = F.normalize(predicted_features, dim=-1)
        text_features_norm = F.normalize(text_features, dim=-1)[None]

        logits = torch.einsum('bnc,bkc->bkn', predicted_features_norm, text_features_norm)
        class_preds = logits.argmax(1)
        class_preds = torch.tensor(class_mapping_clip)[class_preds]
        logits = max_logits_per_class(logits, class_mapping_clip)
...
def main(local_rank, args):
    ...
    if clip_features:
        ...
        if args.text_embeddings_path is not None and os.path.exists(args.text_embeddings_path):
            ...
            if type(text_features) in [tuple, list]:
                ...
                if isinstance(class_mapping_clip, torch.Tensor):
                    class_mapping_clip = class_mapping_clip.tolist()
                else:
    ...
    # printing frequency
    print_freq = cfg.print_freq if not args.debug else 1
    try:
    ...
    with torch.no_grad():
        for i_iter_val, loaded_data in enumerate(used_dataloader):
            ...
            _outputs_vox_clip = assign_clip_labels(
                ...
                text_features, None, None, assignment_only=True, class_offset=CLASS_OFFSET)
            ...
            if clip_features:
                ...
                with torch.no_grad():
                    ...
                    loss_ce_clip, loss_lovasz_clip, non_ignore, outputs_pts_clip, targets, val_grid_fts_int = assign_clip_labels(
                        ...
                        class_offset=CLASS_OFFSET, compute_loss=False)
                    if args.show:
                        fig = plt.figure()
                        show3d(val_grid_fts_int[non_ignore].cpu(), fig, 1, 2, 1, s=0.5,
                               labels=outputs_pts_clip.squeeze().cpu() + 1, title='assignments of predicted features',
                    ...
                    # evaluate voxels
                    # a) all voxels
                    miou=CalMeanIou_vox_clip_all._after_step(outputs_vox_clip.cpu(), val_vox_label_cls_val, return_cur_miou=args.debug)
...
def assign_clip_labels(args, class_mapping_clip, loss_func_noW, lovasz_softmax, outputs_pts_fts, text_features,
    ...
    outputs_pts_clip, logits_clip = assign_labels_clip(
        outputs_pts_fts.float().cuda(), text_features, class_offset=class_offset, maskclip=args.maskclip,
    ...
    targets = (train_pt_labs_fts[non_ignore]).unsqueeze(0) #+ class_offset
