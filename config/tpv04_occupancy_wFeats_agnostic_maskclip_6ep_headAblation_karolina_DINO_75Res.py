_base_ = [
    # './_base_/dataset_mini_wFeats_agnostic.py',
    './_base_/dataset_100_agnostic_wFeats.py',
    './_base_/optimizer.py',
    './_base_/schedule.py',
]

scale_rate = 0.75
class_weights_path = "/scratch/project/open-26-3/vobecant/projects/TPVFormer-OpenSet/data/class_weights.pth"
max_epochs = 6


dataset_params = dict(
    version="v1.0-trainval",
    ignore_label=255,
    fill_label=0,
    fixed_volume_space=True,
    label_mapping="./config/label_mapping/nuscenes-agnostic.yaml",
    label_mapping_gt="./config/label_mapping/nuscenes-noIgnore.yaml",
    max_volume_space=[51.2, 51.2, 3],
    min_volume_space=[-51.2, -51.2, -5],
    # features_path="/nfs/datasets/nuscenes/maskclip_features_projections_all",
    # projections_path="/nfs/datasets/nuscenes/features/projections",
    features_path="/mnt/proj1/open-26-3/datasets/nuscenes/maskclip_features_projections_all",
    projections_path="/mnt/proj1/open-26-3/datasets/nuscenes/features/projections",
    features_type="clip",
    dino_features=True,
    features_type_dino="k",
    # features_path_dino="/nfs/datasets/nuscenes/features/vit_small_8/matched",
    # projections_path_dino="/nfs/datasets/nuscenes/features/projections",
    features_path_dino="/mnt/proj1/open-26-3/datasets/nuscenes/features/vit_small_8/matched",
    projections_path_dino="/mnt/proj1/open-26-3/datasets/nuscenes/features/projections",
    # max_features = 10000
)

occupancy = True
lovasz_input = 'voxel'
ce_input = 'voxel'
voxel_feature_loss = 'l2'

feature_learning = True

point_cloud_range = [-51.2, -51.2, -5.0, 51.2, 51.2, 3.0]

_dim_ = 256
_pos_dim_ = _dim_ // 2
_ffn_dim_ = _dim_ * 2
_num_levels_ = 4
_num_cams_ = 6
tpv_h_ = 100
tpv_w_ = 100
tpv_z_ = 8
scale_h = 1
scale_w = 1
scale_z = 1
grid_size = [tpv_h_ * scale_h, tpv_w_ * scale_w, tpv_z_ * scale_z]
num_points_in_pillar = [4, 32, 32]
num_points = [8, 64, 64]
nbr_class = 2  # 18

# feature learning
feature_dim = 512  # 768
hidden_dims_ft = 1024
out_dims_ft = hidden_dims_ft

# number of layers of the decoders
dec_layers_occupancy = 2
dec_layers_features = 2

# feature with DINO
feature_dim_dino = 384  # 768
hidden_dims_ft_dino = 1024
out_dims_ft_dino = hidden_dims_ft_dino

model = dict(
    type='TPVFormer',
    use_grid_mask=True,
    tpv_aggregator=dict(
        type='TPVAggregator',
        tpv_h=tpv_h_,
        tpv_w=tpv_w_,
        tpv_z=tpv_z_,
        nbr_classes=nbr_class,
        in_dims=_dim_,
        hidden_dims=2 * _dim_,
        out_dims=_dim_,
        scale_h=scale_h,
        scale_w=scale_w,
        scale_z=scale_z,
        feature_dim=feature_dim,
        hidden_dims_ft=hidden_dims_ft,
        out_dims_ft=out_dims_ft,
        dec_layers_occupancy=dec_layers_occupancy,
        dec_layers_features=dec_layers_features,
        feature_dim_dino=feature_dim_dino,
        hidden_dims_ft_dino=hidden_dims_ft_dino,
        out_dims_ft_dino=out_dims_ft_dino,
    ),
    img_backbone=dict(
        type='ResNet',
        depth=101,
        num_stages=4,
        out_indices=(1, 2, 3),
        frozen_stages=1,
        norm_cfg=dict(type='BN2d', requires_grad=False),
        norm_eval=True,
        style='caffe',
        dcn=dict(type='DCNv2', deform_groups=1, fallback_on_stride=False),
        stage_with_dcn=(False, False, True, True)),
    img_neck=dict(
        type='FPN',
        in_channels=[512, 1024, 2048],
        out_channels=_dim_,
        start_level=0,
        add_extra_convs='on_output',
        num_outs=4,
        relu_before_extra_convs=True),
    tpv_head=dict(
        type='TPVFormerHead',
        tpv_h=tpv_h_,
        tpv_w=tpv_w_,
        tpv_z=tpv_z_,
        pc_range=point_cloud_range,
        num_feature_levels=_num_levels_,
        num_cams=_num_cams_,
        embed_dims=_dim_,
        positional_encoding=dict(
            type='LearnedPositionalEncoding',
            num_feats=_pos_dim_,
            row_num_embed=tpv_h_,
            col_num_embed=tpv_w_),
        encoder=dict(
            type='TPVFormerEncoder',
            tpv_h=tpv_h_,
            tpv_w=tpv_w_,
            tpv_z=tpv_z_,
            num_layers=3,
            pc_range=point_cloud_range,
            num_points_in_pillar=num_points_in_pillar,
            return_intermediate=False,
            transformerlayers=dict(
                type='TPVFormerLayer',
                attn_cfgs=[
                    dict(
                        type='TPVCrossViewHybridAttention',
                        embed_dims=_dim_,
                        num_levels=1),
                    dict(
                        type='TPVImageCrossAttention',
                        pc_range=point_cloud_range,
                        deformable_attention=dict(
                            type='TPVMSDeformableAttention3D',
                            embed_dims=_dim_,
                            num_points=num_points,
                            num_z_anchors=num_points_in_pillar,
                            num_levels=_num_levels_,
                            floor_sampling_offset=False,
                            tpv_h=tpv_h_,
                            tpv_w=tpv_w_,
                            tpv_z=tpv_z_,
                        ),
                        embed_dims=_dim_,
                        tpv_h=tpv_h_,
                        tpv_w=tpv_w_,
                        tpv_z=tpv_z_,
                    )
                ],
                feedforward_channels=_ffn_dim_,
                ffn_dropout=0.1,
                operation_order=('self_attn', 'norm', 'cross_attn', 'norm',
                                 'ffn', 'norm')))), )
