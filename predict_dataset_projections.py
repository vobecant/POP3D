...
from predict_dataset_all import process_clip
...
def main(local_rank, args):
    ...
    CalMeanIou_pts_visible = MeanIoU(unique_label_cls, ignore_label_semantic, unique_label_str_cls, 'pts_visible')
    CalMeanIou_pts_all = MeanIoU(unique_label_cls, ignore_label_semantic, unique_label_str_cls, 'pts_all')
    CalMeanIou_pts_all_agn = MeanIoU([0, 1], 255, unique_label_str_agn, 'pts_all_agn')
    ...
    with torch.no_grad():
        for i_iter_val, loaded_data in enumerate(val_dataset_loader):
            ...
            if not args.projections_only:
                ...
                predicted_labels_allVox_visiblePts, predicted_labels_allPts, prediction_allPts_fts = model_occ(img=imgs,
                                                                                                               img_metas=img_metas,
                                                                                                               points=val_grid_float_ft.clone())

            if args.projections_only:
                voxels_with_projections, _idx = torch.unique(val_grid_int[0, matching_points[0]], dim=0,
                                                             return_inverse=True)
                features_voxels_with_projections = []
                for _i in range(voxels_with_projections.shape[0]):
                    cur = torch.mean(val_fts[0, _idx == _i], dim=0)
                    features_voxels_with_projections.append(cur)
                features_voxels_with_projections = torch.stack(features_voxels_with_projections).unsqueeze(0)

                location_points_with_projections = val_grid_float_ft  # [0,matching_points[0]]
                features_points_with_projections = val_fts

                voxels_with_projections = voxels_with_projections.unsqueeze(0)
                predicted_labels_visiblePts, predicted_labels_allPts, predicted_labels_allVox_predOccPts, predicted_labels_allVox_allPts = process_clip(
                    args, class_mapping_clip, voxels_with_projections.long(),
                    features_voxels_with_projections, features_voxels_with_projections,
                    val_fts, val_fts,
                    text_features, val_grid,
                    voxels_with_projections,
                    val_vox_label_cls,
                    matching_points[0])
                predicted_labels_allVox_visiblePts = predicted_labels_allVox_predOccPts

            elif feature_learning or args.linear_probe or clip_features:

                # we are also predicting the features
                if feature_learning:
                    # get the occupied voxels; for these voxels, get the feature predictions
                    occupied_voxels_loc = torch.stack(torch.where(predicted_labels_allVox_visiblePts[0].argmax(0))).T
                ...
                if clip_features:
                    predicted_labels_allPts, predicted_labels_allVox_visiblePts, predicted_labels_visiblePts = process_clip(
                        args, class_mapping_clip, input_loc, outputs_vox_unq_fts, predicted_labels_allPts,
                        predicted_labels_allVox_visiblePts, prediction_allPts_fts, text_features, val_grid,
                        val_grid_float,
                        val_grid_fts_gt, val_grid_int, val_vox_label_cls)
                else:
                    ...
                    with torch.no_grad():
                        ...
                        predicted_labels_allVox_visiblePts = np.ones_like(val_vox_label_cls)[0] * FREE_LABEL_SEMANTIC
                        predicted_labels_allVox_visiblePts = torch.from_numpy(
                            nb_process_label(predicted_labels_allVox_visiblePts, label_voxel_pair)).unsqueeze(
                            0).unsqueeze(-1).cuda()
            ...
            predict_labels_vox_agn = torch.zeros_like(predicted_labels_allVox_visiblePts)
            predict_labels_vox_agn[predicted_labels_allVox_visiblePts < FREE_LABEL_SEMANTIC] = 1

            val_pt_labs_agn = torch.zeros_like(val_pt_labs, device=val_pt_labs.device)
            val_pt_labs_agn[val_pt_labs < FREE_LABEL_SEMANTIC] = 1
            predict_labels_pts_agn = torch.zeros_like(predicted_labels_allPts,
                                                      device=predicted_labels_allPts.device)
            # [matching_points[0]].unsqueeze(0)
            predict_labels_pts_agn[predicted_labels_allPts < FREE_LABEL_SEMANTIC] = 1
            ...
            if save_dir is not None:
                ...
                save_dict = {
                    ...
                    'predicted_labels_allVox': predicted_labels_allVox_visiblePts.cpu().numpy(),
                }
            ...
            for count in range(len(val_grid_int)):
                ...
                CalMeanIou_pts_visible._after_step(predicted_labels_visiblePts[count].squeeze().cuda(),
                                                   val_pt_labs_cls.cuda()[count, matching_points[0]].squeeze())
                CalMeanIou_pts_all._after_step(predicted_labels_allPts[count].squeeze().cuda(),
                                               val_pt_labs_cls.cuda()[count].squeeze())
                CalMeanIou_pts_all_agn._after_step(predict_labels_pts_agn[count].squeeze().cuda(),
                                                   val_pt_labs_agn.cuda()[count].squeeze())

                CalMeanIou_vox._after_step(predicted_labels_allVox_visiblePts[
                ...
                if args.unique_voxels:
                    ...
                    CalMeanIou_vox_unq._after_step(predicted_labels_allVox_visiblePts[
                                                       count, locations_for_vox[cur_indices, 1], locations_for_vox[
                                                           cur_indices, 2], locations_for_vox[
                                                           cur_indices, 3]].flatten().cuda(),
    ...
    val_miou_pts_all_agn = CalMeanIou_pts_all_agn._after_epoch()
    val_miou_pts_all = CalMeanIou_pts_all._after_epoch()
    ...
    logger.info('*** POINTS ***\n')
    logger.info('Current val miou pts *with projections to the image* is %.3f\n' % (val_miou_pts_visible))
    logger.info('Current val miou pts all (w/o projections are considered empty) is %.3f\n' % (val_miou_pts_all))
    logger.info(
        'Current val miou pts all (w/o projections are considered empty) AGNOSTIC is %.3f\n\n' % (val_miou_pts_all_agn))
    ...
    if global_rank == 0:
        ...
        torch.save(CalMeanIou_pts_visible.confusion_matrix.cpu(), os.path.join(output_dir, 'pts_conf.pth'))
        torch.save(CalMeanIou_pts_all_agn.confusion_matrix.cpu(), os.path.join(output_dir, 'pts_agn_conf.pth'))
...
# def process_clip(args, class_mapping_clip, input_loc, outputs_vox_unq_fts, predicted_labels_allPts,
#                  predicted_labels_allVox_predOcc, prediction_allPts_fts, text_features, val_grid, val_grid_float,
#                  val_grid_fts_gt, val_grid_int, val_vox_label_cls):
...
#     return predicted_labels_allPts, predicted_labels_allVox_predOcc, predicted_labels_visiblePts
...
if __name__ == '__main__':
    ...
    args.ckpt_path = str2none(args.ckpt_path)
    args.weights_path_linear = str2none(args.weights_path_linear)
