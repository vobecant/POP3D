...
def main(local_rank, args):
    ...
    try:
        ...
        agnostic = False
    agnostic = False
    print('WARNING: SETTING NON-AGNOSTIC BY HAND!!!')
    ...
    with torch.no_grad():
        for i_iter_val, loaded_data in enumerate(used_dataloader):
            ...
            if not feature_learning:
                imgs, img_metas, val_vox_label_semantic, val_grid, val_pt_labs, val_vox_label_cls = loaded_data
                val_pt_fts = None
                val_grid_fts = None
            else:
            ...
            # assert False, "I might be preparing the 'val_vox_label_cls_val' incorrectly in case that I assume that the empty label is 17."
            ...
            if i_iter_val == 0:
                print(f'val_pt_labs.unique(): {val_pt_labs.unique()}, '
                      f'val_pt_labs_agnostic.unique(): {val_pt_labs_agnostic.unique()}')
            ...
            # TODO: compute mIoU with this general prediction
            # evaluate voxels
            ...
            # a) all voxels
            CalMeanIou_vox_clip_all._after_step(outputs_vox_clip_all.cpu(), val_vox_label_cls_val)
            ...
            _outputs_pts_clip_gtOcc = assign_clip_labels(
                ...
                text_features, None, None, assignment_only=True)
            CalMeanIou_pts_clip._after_step(_outputs_pts_clip_gtOcc.cpu().squeeze(), val_pt_labs.cpu().squeeze())
            CalMeanIou_pts_clip_ignore0._after_step(_outputs_pts_clip_gtOcc.cpu().squeeze(),
                                                    val_pt_labs.cpu().squeeze())
            CalMeanIou_pts_clip_visible._after_step(_outputs_pts_clip_gtOcc[:, matching_points[0]].cpu().squeeze(),
            ...
            CalMeanIou_pts_clip_wRoad._after_step(_outputs_pts_clip_gtOcc_wRoad.cpu().squeeze(),
                                                  val_pt_labs.cpu().squeeze())
            ...
            CalMeanIou_pts_clip_gt._after_step(_outputs_pts_clip_projFt.squeeze().cpu(), val_pt_labs.cpu().squeeze())
            CalMeanIou_pts_clip_visible_gt._after_step(_outputs_pts_clip_projFt[:, matching_points[0]].cpu().squeeze(),
                                                       val_pt_labs[:, matching_points[0]].cpu().squeeze())
            ...
            predict_labels_pts_agn = predict_labels_pts_agn.detach().cpu()
            val_pt_labs = val_pt_labs.squeeze(-1).cpu()

            predict_labels_vox_agn = torch.argmax(predict_labels_vox_agn, dim=1)
            predict_labels_vox_agn = predict_labels_vox_agn.detach().cpu()
            ...
            for count in range(len(val_grid_int)):
                ...
                CalMeanIou_pts_agnostic._after_step(predict_labels_pts_agnostic[count],
                                                    gt_labels_pts_agnostic[count, :, 0].cpu())
    ...
    if clip_features:
        ...
        if clip_features:
            print(f'val_miou_pts_clip (evaluation at GT points, predicted features): {val_miou_pts_clip}')
            print(f'val_miou_pts_clip (evaluation at *VISIBLE* GT points, predicted features): '
        ...
        print(f'CalMeanIou_pts_clip_wRoad: {val_miou_pts_clip_wRoad}')
        print(f'val_miou_pts_clip_ignore0: {val_miou_pts_clip_ignore0}')
...
#
# for info in train_dataset_loader.dataset.imagepoint_dataset.nusc_infos:
#     lidar_sd_token = train_dataset_loader.dataset.imagepoint_dataset.nusc.get('sample', info['token'])['data']['LIDAR_TOP']
#     lidarseg_labels_filename = os.path.join(train_dataset_loader.dataset.imagepoint_dataset.data_path, train_dataset_loader.dataset.imagepoint_dataset.nusc.get('lidarseg', lidar_sd_token)['filename'])
