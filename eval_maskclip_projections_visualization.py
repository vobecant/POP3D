...
from qualitative_results import get_clip_utils, res2txt
...
def main(local_rank, args):
    ...
    print(f'clip_features: {clip_features}')
    ...
    print(f'Dataset size: {len(val_dataset_loader)}')
        ...
        for i_iter_val, loaded_data in enumerate(val_dataset_loader):
            ...
            if clip_features:
                ...
                with torch.no_grad():
                    ...
                    # projected points
                    outputs_pts_clip = assign_labels_clip(val_pt_fts.cuda().float(), text_features.cuda(), 1, True,
                                                          class_mapping_clip, ignore_label=0)[0]
                    ...
                    xyz = val_grid_fts[0]
                    labels = outputs_pts_clip[0]
                    token = img_metas[0]['token'] # change
                    save_dir_cur = os.path.join(args.save_dir, token)
                    if not os.path.exists(save_dir_cur):
                        os.makedirs(save_dir_cur)
                    out_path = os.path.join(save_dir_cur, f'pred.txt')
                    res2txt(xyz, labels, colors, out_path=out_path, verbose=False)
