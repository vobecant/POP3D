    ...
    with torch.no_grad():
        for i_iter_val, loaded_data in enumerate(val_dataset_loader):
            ...
            if args.projected_features:
                ...
                predict_labels_vox, predict_labels_pts, predict_fts_vox, predict_fts_pts = my_model(img=imgs,
                                                                                                    img_metas=img_metas,
            ...
            if clip_features:
                ...
                with torch.no_grad():
                    ...
                    _outputs_vox_clip = assign_clip_labels(
                        ...
                        text_features, val_grid_fts, val_vox_label_cls, assignment_only=True)
                    outputs_vox_clip = torch.ones_like(val_vox_label_cls,
                                                       device=predicted_features_occupied_vox.device) * EMPTY_SEMANTIC_LABEL
                    evaluated_voxels_int = occupied_voxels_loc.long()[1:]
                    outputs_vox_clip[:, evaluated_voxels_int[0], evaluated_voxels_int[1],
                    evaluated_voxels_int[2]] = _outputs_vox_clip
                    if args.show:
                        fig = plt.figure()
                        show3d(evaluated_voxels_int.cpu(), fig, 1, 2, 1, s=0.5,
                               labels=_outputs_vox_clip.squeeze().cpu(), title='assignments predicted occupied voxels',
                               colors=CLASS_COLORS)  # _TINY)
                        show3d(val_grid_fts_int[non_ignore].cpu(), fig, 1, 2, 2, s=0.5,
                               title='GT labels',
                               labels=targets.squeeze().cpu() + 1,
                    ...
                    loss_ce_clip_gt, loss_lovasz_clip_gt, non_ignore, outputs_pts_clip_gt, targets, val_grid_fts_int = assign_clip_labels(
                        ...
                        val_grid_fts, val_vox_label_cls, ignore_label=IGNORE_LABEL_SEMANTIC)
                    if global_rank == 0 and not args.no_wandb:
                        wandb.log({"loss/val_ce_clip_gt": loss_ce_clip}, commit=False)
                        wandb.log({"loss/val_lovasz_clip_gt": loss_lovasz_clip}, commit=False)
                    if args.show:
                        fig = plt.figure()
                        show3d(val_grid_fts_int[non_ignore].cpu(), fig, 1, 2, 1, s=0.5,
                    ...
                    # assert False, "We need to change the targets!"
            ...
            predict_labels_vox_agnostic = predict_labels_vox
            gt_labels_pts_agnostic = val_pt_labs
            predict_labels_pts_agnostic = predict_labels_pts
