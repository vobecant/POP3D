...
def linear_probe_pred_setup2(local_rank, args, cfg, use_lovasz, weights_path):
    # global settings
    torch.backends.cudnn.benchmark = True
    ...
    num_epochs = cfg['max_epochs']
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    model_linear_probe = build_linear_probe(cfg['model_params']).to(device)
    n_parameters_lin = sum(p.numel() for p in model_linear_probe.parameters() if p.requires_grad)
    assert n_parameters_lin > 0, "We need to train the linear probe!"
    logger.info(f'Number of trainable params of the linear probe: {n_parameters_lin}')
    if distributed and n_parameters_lin > 0:
        find_unused_parameters = cfg.get('find_unused_parameters', False)
        ddp_model_module = torch.nn.parallel.DistributedDataParallel
        model_linear_probe = ddp_model_module(
            model_linear_probe.cuda(),
            device_ids=[torch.cuda.current_device()],
            broadcast_buffers=False,
            find_unused_parameters=find_unused_parameters)
    else:
        model_linear_probe = model_linear_probe.cuda()
    ...
    model_occ_and_fts = model_builder.build(cfg.model)
    for param in model_occ_and_fts.parameters():
        param.requires_grad = False
    n_parameters = sum(p.numel() for p in model_occ_and_fts.parameters() if p.requires_grad)
    assert n_parameters == 0, "Network predicting occupancy and features has to be frozen!"
    logger.info(f'Number of trainable params of the occupancy network: {n_parameters}')
    if distributed and n_parameters > 0:
        find_unused_parameters = cfg.get('find_unused_parameters', False)
        ddp_model_module = torch.nn.parallel.DistributedDataParallel
        model_occ_and_fts = ddp_model_module(
            model_occ_and_fts.cuda(),
            device_ids=[torch.cuda.current_device()],
            broadcast_buffers=False,
            find_unused_parameters=find_unused_parameters)
    else:
        model_occ_and_fts = model_occ_and_fts.cuda()

    print('done ddp occupancy&features model')
    ...
    # if 'state_dict' in ckpt:
    #     ckpt = ckpt['state_dict']
    # print(model_linear_probe.load_state_dict(revise_ckpt_linear_probe(ckpt, ddp=distributed and n_parameters_lin > 0), strict=True))
    # print(f'successfully loaded ckpt to linear probe')
    ...
    if 'state_dict' in ckpt:
        ckpt = ckpt['state_dict']
    print(model_occ_and_fts.load_state_dict(revise_ckpt_linear_probe(ckpt, ddp=distributed and n_parameters > 0),
                                            strict=True))
    print(f'successfully loaded ckpt to model_occ_and_fts')

    model_occ_and_fts.eval()

    n_parameters_occ = sum(p.numel() for p in model_occ_and_fts.parameters() if p.requires_grad)
    logger.info(f'Number of trainable params in the occupancy model: {n_parameters_occ}')
    assert n_parameters_occ == 0
    n_parameters = sum(p.numel() for p in model_linear_probe.parameters() if p.requires_grad)
    logger.info(f'Number of trainable params in the linear head: {n_parameters}')
    ...
    # get optimizer, loss, scheduler
    optimizer = build_optimizer(model_linear_probe, cfg.optimizer)
    loss_func, lovasz_softmax = loss_builder.build(ignore_label=ignore_label)
    num_iters = len(train_dataset_loader) * num_epochs
    scheduler = CosineLRScheduler(
        optimizer,
    ...
    for epoch in range(num_epochs):
        ...
        if not args.eval_only:
            ...
            for i_iter, loaded_data in enumerate(iterable):
                ...
                fwd_s = time.time()
                if args.setup == 'setup2a':
                    train_grid_fts = torch.stack(torch.where(torch.ones(train_grid.shape[1:]))).T
                    with torch.no_grad():
                        predict_labels_vox, predict_labels_pts, predict_fts_pts = model_occ_and_fts(
                            img=imgs, img_metas=img_metas, points=train_grid, features=train_grid_fts)


                elif args.setup == 'setup2b':
                    points = train_grid_fts_gt.cuda().float()
                    train_grid_fts = train_grid_fts_gt_int.cuda()
                    with torch.no_grad():
                        predict_labels_vox, predict_labels_pts, predict_fts_pts = model_occ_and_fts(
                            img=imgs, img_metas=img_metas, points=points, features=train_grid_fts)

                    # get the predicted occupied labels
                    for bi in range(predict_labels_vox.shape[0]):
                        # get boolean mask of occupied labels
                        pred_occupied_mask = (predict_labels_pts[bi].argmax(0) == 1).squeeze()
                        # take only those features that are occupied even in the GT
                        pred_occupied_fts = predict_fts_pts[bi][pred_occupied_mask]
                        # take the corresponding labels
                        pred_occupied_idx = train_grid_fts_gt_int[bi][pred_occupied_mask]
                        pred_occupied_targets = train_vox_label_cls[bi][
                            pred_occupied_idx[:, 0], pred_occupied_idx[:, 1], pred_occupied_idx[:, 2]].cuda()
                else:
                    raise Exception(f'Unknown setup "{args.setup}"!')

                pred_occupied_fts = pred_occupied_fts.T.unsqueeze(0)

                # get predictions for the points with GT that were predicted as occupied
                predict_occupied_points_lbl = model_linear_probe(pred_occupied_fts)
                predicted_labels = predict_occupied_points_lbl.argmax(1)
                fwd_t = time.time() - fwd_s

                # loss
                loss_ce = loss_func(predict_occupied_points_lbl, pred_occupied_targets.unsqueeze(0).cuda())
                if log_wandb: wandb.log({'loss/train/ce': loss_ce}, commit=False)
                loss_lovasz = 0
                if use_lovasz:
                    loss_lovasz = lovasz_softmax(
                        torch.nn.functional.softmax(predict_occupied_points_lbl, dim=1).squeeze(),
                        pred_occupied_targets.squeeze(), ignore=ignore_label
                    )
                    if log_wandb: wandb.log({'loss/train/lovasz': loss_lovasz}, commit=False)
                loss = loss_ce + loss_lovasz
                ...
                optimizer.zero_grad()
                loss.backward()
                grad_norm = torch.nn.utils.clip_grad_norm_(model_linear_probe.parameters(), cfg.grad_max_norm)
                optimizer.step()
                loss_list.append(loss.item())
                scheduler.step_update(global_iter)
                bwd_t = time.time() - bwd_s

                # for evaluation and visualization, we need to set the labels at GT-occupied positions as "empty" in the predictions
                targets_full = train_vox_label_cls[:, train_grid_fts_gt_int[..., 0], train_grid_fts_gt_int[..., 1],
                               train_grid_fts_gt_int[..., 2]].squeeze()
                predictions_full = torch.ones_like(targets_full, dtype=targets_full.dtype) * 17
                predictions_full[pred_occupied_mask] = predicted_labels.to(predictions_full.device)

                if args.show:
                    # xyz_pred = torch.where(predict_labels_vox[0].argmax(0))[0].T
                    # xyz_gt = torch.where(train_vox_label[0].flatten())[0].T
                    fig = plt.figure()
                    show3d(pred_occupied_idx, fig, 1, 1, 1, s=1., labels=predicted_labels.cpu())
                    plt.show()
                    fig = plt.figure()
                    show3d(pred_occupied_idx, fig, 1, 2, 1, s=0.1, labels=predicted_labels.cpu())
                    show3d(train_grid_fts_gt_int[0], fig, 1, 2, 2, s=0.1, labels=targets_full)
                    plt.show()

                CalMeanIou_pts_train._after_step(predictions_full, targets_full)
        ...
        with torch.no_grad():
            for i_iter, loaded_data in enumerate(iterable):
                ...
                # get occupancy and features
                with torch.no_grad():
                    predict_labels_vox, predict_labels_pts, predict_fts_pts = model_occ_and_fts(img=imgs,
                                                                                                img_metas=img_metas,
                                                                                                points=val_grid,
                                                                                                features=val_grid_fts)
                dino_feats_list = []
                ...
                # don't get gradients since we do not train anything
                with torch.no_grad():
                    predict_occupied_points_lbl = model_linear_probe(dino_feats)
                fwd_t = time.time() - fwd_s

                if args.show:
                    locs = xyz_pred_occ[0].cpu()
                    labels = predict_occupied_points_lbl[0].argmax(0).cpu()
                ...
                # loss
                loss_ce = loss_func(predict_occupied_points_lbl, dino_targets)
                if log_wandb: wandb.log({'loss/val/ce': loss_ce}, commit=False)
                loss_lovasz = 0
                if use_lovasz:
                    loss_lovasz = lovasz_softmax(
                ...
                predicted_idx_occupied = predict_occupied_points_lbl.argmax(1).flatten().detach().cpu()
            ...
            if val_miou > best_miou_val:
                ...
                best_miou_val = val_miou
                dict_to_save = {
                    'state_dict': model_linear_probe.state_dict(),
                    'optimizer': optimizer.state_dict(),
                    'scheduler': scheduler.state_dict(),
                    'epoch': epoch + 1,
                    'global_iter': global_iter,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--setup', type=str, default='setup2b')
    ...
    elif args.setup.lower().startswith('setup2'):
        torch.multiprocessing.spawn(linear_probe_pred_setup2,
                                    args=(args, cfg, args.use_lovasz, args.weights_path_linear,), nprocs=args.gpus)
