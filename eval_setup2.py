...
def main(local_rank, args):
    ...
    print(model_occ_and_fts.load_state_dict(revise_ckpt(ckpt), strict=True))
    ...
    with torch.no_grad():
        for i_iter_val, loaded_data in enumerate(val_dataset_loader):
            ...
            predict_occ_vox, predict_occ_pts, *_ = model_occ_and_fts(img=imgs, img_metas=img_metas, points=val_grid_float)
            ...
            predict_labels_vox = np.ones_like(val_vox_label_cls)[0] * 17
            predict_labels_vox = torch.from_numpy(nb_process_label(predict_labels_vox, label_voxel_pair)).unsqueeze(
                0).unsqueeze(-1).cuda()

            val_pt_labs_wProj = val_vox_label_cls[:, val_grid_fts_gt_int[..., 0], val_grid_fts_gt_int[..., 1],
                                val_grid_fts_gt_int[..., 2]].squeeze(0)
            predict_labels_pts = torch.ones_like(val_pt_labs_wProj).cuda() * 17  # initialize with "empty" predictions
            ...
            locations_for_vox = val_grid_int if not args.unique_voxels else torch.stack(
                torch.where(val_vox_label_cls != 17)).T

            if args.agnostic:
                predict_labels_vox_agn = copy.deepcopy(predict_occ_vox)
                val_pt_labs_agn = copy.deepcopy(val_pt_labs)
                predict_labels_pts_agn = copy.deepcopy(predict_occ_pts)
                ...
                predict_labels_vox_agn[predict_labels_vox != 17] = 1
                ...
                predict_labels_pts_agn[predict_labels_pts == 17] = 0
            ...
            for count in range(len(val_grid_int)):
                ...
                CalMeanIou_vox._after_step(predict_labels_vox[
                                               ...
                                               val_grid_int[count][:, 2]].flatten(), val_pt_labs[count].flatten())
                ...
                if args.unique_voxels:
                    ...
                    CalMeanIou_vox_unq._after_step(predict_labels_vox[
                                                       count, locations_for_vox[cur_indices, 1], locations_for_vox[
                                                           cur_indices, 2], locations_for_vox[
                                                           cur_indices, 3]].flatten().cuda(), vox_label[
                                                       count, locations_for_vox[cur_indices, 1], locations_for_vox[
                                                           cur_indices, 2], locations_for_vox[
                                                           cur_indices, 3]].flatten().cuda())
