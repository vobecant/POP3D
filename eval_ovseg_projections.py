...
ODISE_PREDS_DIR = '/home/vobecant/PhD/ODISE/predictions_nusc_val'
...
def main(local_rank, args):
    ...
    if clip_features:
        CalMeanIou_pts_clip = MeanIoU(unique_label_clip, 0, unique_label_str_clip, 'pts_clip')
        CalMeanIou_pts_clip_visible = MeanIoU(unique_label_clip, 0, unique_label_str_clip,
                                              'pts_clip_visible')
    ...
    with torch.no_grad():
        for i_iter_val, loaded_data in enumerate(val_dataset_loader):
            ...
            if not feature_learning:
                imgs, img_metas, val_vox_label_agnostic, val_grid, targets_0based, val_vox_label_cls = loaded_data
                predictions_ovseg_full = None
                val_grid_fts = None
            else:
                imgs, img_metas, val_vox_label_agnostic, val_grid, targets_0based, val_vox_label_cls, val_grid_fts, predictions_ovseg_full, val_vox_label_cls_val, matching_points, _ = loaded_data

            val_grid_int = val_grid.to(torch.long).cuda()
            targets_0based = targets_0based.cuda()
            predictions_ovseg_full = predictions_ovseg_full.cuda().long()
            predictions_ovseg_remapped_0based = class_mapping_clip[predictions_ovseg_full]

            # projected points
            outputs_pts_clip = predictions_ovseg_remapped_0based[0].long()
            outputs_pts_clip_all = torch.ones_like(targets_0based) * 0  # EMPTY_SEMANTIC_LABEL
            outputs_pts_clip_all[0, matching_points[0], 0] = outputs_pts_clip
            CalMeanIou_pts_clip._after_step(outputs_pts_clip_all.squeeze(), targets_0based.squeeze())
            CalMeanIou_pts_clip_visible._after_step(outputs_pts_clip.squeeze(),
                                                    targets_0based[:, matching_points[0]].squeeze())
            ...
            if False:
                if i_iter_val % print_freq_wandb_val == 0 and (args.no_dist or dist.get_rank() == 0):
                    if VIS and not args.no_wandb:
                        ...
                        gt_fts = predictions_ovseg_full[0] if feature_learning else None
                        pred_fts = None
