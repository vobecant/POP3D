...
class BaseSegmentor(BaseModule, metaclass=ABCMeta):
    ...
    def show_result(self,
        ...
        if show:
            if features is None:
                features = torch.load('pretrain/coca_cola_van_ViT16_clip_text.pth', map_location='cpu')
            if not isinstance(features, np.ndarray):
                features = features.numpy()
            sim = np.einsum('bn,nhw->bhw', features, result[0])[0]
            plt.imshow(img[..., ::-1])
            plt.imshow(sim, alpha=0.4, cmap='bwr')
