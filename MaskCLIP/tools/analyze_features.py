...
def parse_args():
    ...
    parser.add_argument('--projections-dir', default="/nfs/datasets/nuscenes/features/projections")
    parser.add_argument('--save-dir', default='/nfs/datasets/nuscenes/maskclip_features_projections_all')
    parser.add_argument('--num-workers', type=int, default=None)
    parser.add_argument('--features-path', type=str, default=None, help='Path to the text features.')
...
def main():
    ...
    with tempfile.TemporaryDirectory() as tmpdirname:
        # link images to this directory
        data_root = '/nfs/datasets/nuscenes/mmseg/images/trainvaltest_flatten'
        with open(args.paths_file, 'r') as f:
            paths = [os.path.join(data_root,p.strip()) for p in f.readlines()]
            for path in paths:
                imname = os.path.split(path)[-1]
                os.symlink(path, os.path.join(tmpdirname, imname))
        cfg.data.test.data_root = '/tmp'
        cfg.data.test.img_dir = os.path.split(tmpdirname)[-1]
        ...
        features = torch.load(args.features_path, map_location='cpu').numpy()
