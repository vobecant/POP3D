...
def main(local_rank, args):
    ...
    if args.compute_upperbound_clip:
        train_grid_fts_int, train_pt_fts = compute_upper_bound_clip(args, class_mapping_clip, feature_learning,
                                                                    text_features, train_dataset_loader_noaug,
                                                                    train_pt_fts, unique_label, unique_label_str,
                                                                    val_dataset_loader)
    ...
    while epoch < max_num_epochs:
        ...
        for i_iter, loaded_data in enumerate(train_dataset_loader):
            time_s = time.time()
            if not feature_learning:
                imgs, img_metas, train_vox_label_occupancy, train_grid, train_pt_labs_agnostic, train_vox_label_cls = loaded_data
                train_grid_fts = None
            else:
            ...
            # print(f'train_grid: {train_grid.max()} {train_grid.cpu().unique()}')
            ...
            if clip_features:
                ...
                with torch.no_grad():
                    ...
                    loss_ce_clip, loss_lovasz_clip, non_ignore, outputs_pts_clip, targets, train_grid_fts_int = assign_clip_labels(
                        ...
                        train_grid_fts, train_vox_label_cls, compute_loss=args.debug)
                    ...
                    with torch.no_grad():
                        ...
                        if n_occ > 0:
                            ...
                            if i_iter % print_freq_wandb_train == 0 and log2wandb:
                                try:
            ...
            if cfg.ce_input.lower() != 'none':
                ...
                if args.debug:
                    print(f'ce_input: {ce_input.shape}, ce_label: {ce_label.shape} {ce_label.unique()}')
                    break
            ...
            for count in range(len(train_grid_int)):
                # points
                CalMeanIou_pts_agnostic._after_step(predict_labels_pts_agnostic[count], train_pt_labs_agnostic[count])
            ...
            if i_iter % print_freq == 0:
                if args.no_dist or dist.get_rank() == 0:
                    ...
                    time_str = '{:.3f} data, {:.3f} forward, {:.3f} optim, {:.3f} metrics, {:.3f} clip, {:.3f} iter'.format(
                        data_iter_time, fwd_t, optim_t, metrics_t, clip_t, pass_iter_time)
            ...
            if i_iter % print_freq_wandb_train == 0 and (args.no_dist or dist.get_rank() == 0):
                if args.debug: print(f'VIS and not args.no_wandb: {VIS and not args.no_wandb}')
                ...
                if args.debug: print('After logging to wandb')
        ...
        if args.debug: print('After training loop.')
        log2wandb = global_rank == 0 and not args.no_wandb
