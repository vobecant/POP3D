...
class DatasetWrapper_NuScenes(data.Dataset):
    def __init__(self, in_dataset, grid_size, fill_label=0, fill_label_gt=None, ignore_label=255,
                 ...
                 semantic_points=False, max_features=None, no_aug=False): 
        ...
        self.no_aug = no_aug # use no augmentations
    ...
    def getitem_feats(self, data):
        ...
        grid_ids_sorted = np.lexsort((grid_ind[:, 0], grid_ind[:, 1], grid_ind[:, 2]))
        label_voxel_pair_cls = label_voxel_pair_cls[grid_ids_sorted, :]
        processed_label_cls = nb_process_label(np.copy(processed_label_cls), label_voxel_pair_cls)
        t_sem = time.time() - s_sem

        # get occlusion targets
        occlusion_targets = np.zeros(self.grid_size, dtype=np.uint8)
        occlusion_targets[(grid_ind[:, 0], grid_ind[:, 1], grid_ind[:, 2])] = 1
