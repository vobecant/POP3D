...
class ImagePoint_NuScenes(data.Dataset):
    def __init__(self, data_path, imageset='train', label_mapping="nuscenes.yaml",
        ...
        with open(imageset, 'rb') as f:
            data = pickle.load(f)
        print(f'Loaded data from {imageset}, data_path={data_path}')
    ...
    def __getitem__(self, index):
        ...
        t_labels = time.time() - s_labels

        s_ret = time.time()
        if not self.retrieval:
            points_label_cls = np.vectorize(self.learning_map_gt.__getitem__)(np.copy(points_label))
            points_label = np.vectorize(self.learning_map.__getitem__)(points_label)
        t_ret = time.time() - s_ret

        if self.class_agnostic:
            points_label[:] = 1

        logger = self.get_logger()
        logger.info('DATA: info: {:.2f}s, cam: {:.2f}s, lidar: {:.2f}s, labels: {:.2f}s'.format(t_info, t_cam, t_lidar, t_labels))
...
class ImagePoint_NuScenes_withFeatures(ImagePoint_NuScenes):
    ...
    def __getitem__(self, index):
        ...
        t_cam_fts = time.time() - s_cam_fts

        s_lid = time.time()
        # Load LiDAR points.
        lidar_path = info['lidar_path']
        points = np.fromfile(lidar_path, dtype=np.float32, count=-1).reshape([-1, 5])
        t_lid = time.time() - s_lid
