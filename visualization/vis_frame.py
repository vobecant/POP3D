...
def get_image_grid(images):
    images_top = np.concatenate(images[:3], axis=1)
    images_bottom = np.concatenate(images[3:], axis=1)
    images = np.concatenate([images_top, images_bottom])
    return images
...
def show3d(points, fig=None, s=0.01, savepath=None, colors=None, labels=None, ft_color=None, ft_loc=None, show_colorbar=False, ax=None, show=False):
